...
"""
from gusto import *
from firedrake import (as_vector, IcosahedralSphereMesh, SpatialCoordinate,
...
def setup_2d_recovery(dirname):
    ...
    mesh = IcosahedralSphereMesh(radius, refinement_level=3)
    ...
    v_CG1_true = Function(VuCG1).interpolate(as_vector([0.0, -x[2], x[1]]))
    ...
    v_Vu = Function(Vu).project(as_vector([0.0, -x[2], x[1]]))
    v_CG1 = Function(VuCG1)

    # make the recoverers and do the recovery
    rho_recoverer = Recoverer(rho_DG0, rho_CG1, VDG=VDG1)
    v_recoverer = Recoverer(v_Vu, v_CG1, VDG=VuDG1)
