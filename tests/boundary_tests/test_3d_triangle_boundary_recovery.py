...
def setup_3d_recovery(dirname):
    ...
    m = RectangleMesh(ncolumnsx, ncolumnsy, L, W, quadrilateral=False)
    ...
    np.random.seed(0)
    expr = np.random.randn() + np.random.randn()*x + np.random.randn()*y + np.random.randn()*z + np.random.randn()*x*z + np.random.randn()*y*z
